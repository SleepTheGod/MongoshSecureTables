// Connect to MongoDB
use searchcheck; // Switch to or create the 'searchcheck' database
...
db.users.createIndex({ email: 1 }, { unique: true }); // Unique index for email
...
db.users.insertMany([
    {
        ...
        password_hash: "hashed_password", // The actual password should be hashed using a secure method
...
db.search_history.createIndex({ search_time: 1 });

db.search_history.insertMany([
    {
        user_id: 1,
        search_query: "data breach",
        search_time: new Date()
    },
    {
        user_id: 2,
        search_query: "cybersecurity best practices",
        search_time: new Date()
    }
]);
...
db.payment_history.createIndex({ payment_date: 1 });
...
db.subscription_plans.createIndex({ plan_name: 1 });
...
db.user_subscriptions.insertMany([
    {
        ...
        plan_id: 1,
        start_date: new Date(),
        end_date: new Date(Date.now() + 30*24*60*60*1000), // Subscription lasts for a month
        status: "active"
    },
    {
        user_id: 2,
